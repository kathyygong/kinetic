...
| Mobile perceived-recovery flow | NLP recovery notes open explicit perceived-recovery capture; captured fields coexist with HealthKit summaries; AI never fabricates readiness values from text | Native explicit controls, review, deterministic merge, HealthKit-field preservation, idempotency, and privacy tests pass; physical-device/web Recovery readback remains |
| Mobile Phase 3 shared check-in contract | [MOBILE_CHECKIN_CONTRACT.md](./MOBILE_CHECKIN_CONTRACT.md), canonical fixture, deterministic recovery/workout application, failure taxonomy, existing-domain payloads, strict auth, privacy-safe audit, and owner-only rules | Part A full Windows suite and Part B 47-test Swift, simulator install/launch, signed generic-device, frontend lint/TypeScript/smoke/build gates pass 2026-07-20; physical-device/live readback and environment-limited backend/emulator reruns remain in [MOBILE_CHECKIN_HANDOFF.md](./MOBILE_CHECKIN_HANDOFF.md) |
...
| iOS package and app compile | `cd ios/KineticCompanion && swift test`; Xcode simulator/device build | 47 Swift tests, unsigned simulator build/install/launch on iPhone 17 / iOS 26.3, and signed generic-device build passed 2026-07-20; connected iPhone was unavailable for the Phase 3 interaction rerun |
